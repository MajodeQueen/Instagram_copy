...
export default function CommentReply({ reply, comment , postId}) {
  ...
  const alreaylikedComment = reply.likes.find(
  ...
  const replyLike = async (replyId) => {
    dispatch({ type: 'LIKE_LOADING' });
    try {
      const result = await axios.post('/api/posts/likeReply', {
        postId,
        commentId,
        replyId
       ,
      ...
      if (result) {
        toast.success('Successful reply like');
      ...
      <div className="flex justify-between  ml-6 items-center  rounded-md">
        ...
        <div className="">
          {loading ? (
            ...
            <BsHeart onClick={() => replyLike(reply._id)} />
          ) : (
            <BsHeart onClick={() =>replyLike(reply._id)} />
          )}
        </div>
